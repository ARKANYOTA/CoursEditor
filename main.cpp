...
void draw_matiere() {
    ...
    if(is_infos && !path_is_dir(path)){
        ...
        cout << "\033[" << j + 4 << ";" << size.ws_col / 2 + 8 << "H\u250f" << borderutf8*(max_len_val+1) << "\u2513" << endl;
        for (auto const&[key, val]: path_info) {
            cout << "\033[" << j + 5 << ";" << size.ws_col / 2 + 8 << "H" << replacorbar << "\u2503\033[0m" << endl;
            cout << "\033[" << j + 5 << ";" << size.ws_col / 2 + 8 << "H\u2503 " << key << ": " << val << "\033[0m" << endl;
            j++;
        }
        cout << "\033[" << j + 5 << ";" << size.ws_col / 2 + 8 << "H\u2517" << borderutf8*(max_len_val+1) << "\u251b" << endl;
    }
...
void execute_file() {
    if (!path_is_dir(current_matiere)) {
        ...
        if (send_confirmation("Etez vous sur de vouloir lancer votre editeur sur ce dossier ?", "Oui lance (y/o)", "Ne pas lancer")) {
...
void draw_interface() {
    ...
    for(auto &notification: list_notifications){
        ...
        cout << "\033[107;30m\033[" << i << ";" << 4 << "H" << notification.first << "  "<< notification.second.first <<"  " << notification.second.second << "\033[0m";
        i--;
    }
}
